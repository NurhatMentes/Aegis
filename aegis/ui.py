...
if edit_mode:
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 🔑 API Bilgileri")
...
if True:
        ...
        <div class="header-container">
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                <div style="color: #64748b; font-size: 0.85rem; font-weight: 500;">
                    Latency: <span style="color: #38bdf8; font-weight: 700;">{latency} ms</span>
                </div>
                <div style="color: #64748b; font-size: 0.85rem; font-weight: 500; padding-right: 15px;">
                    Active Trackers: <span style="color: #f59e0b; font-weight: 700;">{tracker_count}</span>
                </div>
                {ws_status_html}
            </div>
        </div>
        ...
        with left_col:
            ...
            if not log_content:
                log_content = "📡 Radar Scanning... No operational decisions recorded yet."
        ...
        with right_col:
            ...
            if not trackers:
                ...
                <div style="text-align: center; padding: 4rem; background: rgba(30, 41, 59, 0.2); border-radius: 12px; border: 1px dashed #334155; margin-bottom: 2rem;">
                    <h3 style="color: #94a3b8; margin-bottom: 5px;">📡 Radar Scanning... No active positions.</h3>
                    <p style="color: #64748b; font-size: 0.9rem;">Waiting for Skynet to initiate a position or manual entry.</p>
                </div>
                """), unsafe_allow_html=True)
            else:
                for inst_id, t_info in trackers.items():
                    ...
                    if state_val == "INIT":
                        ...
                        sl_ts_html = f'''
                            <div class="target-highlight" style="border-left: 3px solid #3b82f6; background: rgba(59, 130, 246, 0.05);">
                                ...
                                <div style="font-size:0.95rem; font-weight:700; color:#3b82f6;">Stop Level: ${be_px:.6f}</div>
                            </div>
                        '''
                        ...
                        sl_ts_html = f'''
                            <div class="target-highlight" style="border-left: 3px solid #8b5cf6; background: rgba(139, 92, 246, 0.05);">
                                ...
                                <div style="font-size:0.95rem; font-weight:800; color:#8b5cf6;">
                                    Stop Level: ${ts_px:.6f} <span style="font-size:0.8rem; opacity:0.8;">(Mesafe: {ob_mult}x ATR)</span>
                                </div>
                            </div>
                        '''
                    ...
                    <div class="position-card">
                        ...
                        <div class="card-grid-3">
                            ...
                            <div class="metric-box">
                                <div class="metric-label">Spot Move</div>
                        ...
                        <div class="card-grid-3">
                            <div class="metric-box">
                                <div class="metric-label">Live ATR</div>
                                <div class="metric-value" style="color: #38bdf8;">{atr_pct:.4f}%</div>
                            </div>
                            <div class="metric-box">
                                <div class="metric-label">10s Vol Ratio</div>
                                <div class="metric-value">RVOL: {vol_ratio:.2f}x</div>
                            </div>
                            <div class="metric-box">
                                <div class="metric-label">OB Imbalance</div>
                        ...
                        <div style="margin-top: 1rem; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 0.75rem;">
                            <div class="{tp1_style}">
                                ...
                                <div style="font-size:0.95rem; font-weight:700; color:{'#10b981' if state_val != 'INIT' else '#f59e0b'};">
                                    Target: ${tp1:.6f} <span style="font-size:0.8rem; opacity:0.8;">(±%{tp1_pct:.2f})</span>
                                </div>
                            </div>
                            ...
                            <div class="{tp2_style}">
                                ...
                                <div style="font-size:0.95rem; font-weight:700; color:{'#10b981' if state_val == 'TRAILING' else '#3b82f6'};">
                                    Target: ${tp2:.6f} <span style="font-size:0.8rem; opacity:0.8;">(±%{tp2_pct:.2f})</span>
                                </div>
                            </div>
        ...
        <div style="text-align: center; color: #475569; font-size: 0.75rem; margin-top: 1rem; margin-bottom: 2rem;">
            Dashboard refreshed: {datetime.now().strftime("%H:%M:%S.%f")[:-3]} | State file updated: {last_update}
        </div>
        """), unsafe_allow_html=True)
